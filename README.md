...
- **UEFI + BIOS** boot desteği (tek bölümlü yaklaşımda firmware uyumluluğu cihaza göre değişebilir).
...
1. `scripts/build-rootfs.sh`
   ...
   - apt/dpkg, kernel, grub (conflict yaratmayan `*-bin` yaklaşımı), NetworkManager, nmtui gibi bileşenleri kurar.
   - root parolasını `wolf` olarak ayarlar.

2. `scripts/build-image.sh`
   - Tek bölümlü disk imajı oluşturur (MSDOS partition table + tek ext4 bölüm).
